...
*   **Failure Mode:** The "Donate" button logic fails; users might see *nothing* or a broken spinner.
...
*   **Concern:** Routing *all* Indian IPs to Razorpay assumes Razorpay accepts *all* payments smoothly.
...
*   **Correction:** Phase 2 must explicitly state: "Deploy GitHub Sponsors badge *only after* approval email is received."
...
3.  **Data Minimization:**
    - Ensure the IP Geo-lookup is *client-side only* (browser calls API directly). Do not proxy this through your own backend unless you want to deal with GDPR/CCPA compliance for logging user IPs.
...
*Sr. Principal SDE, Infrastructure*
